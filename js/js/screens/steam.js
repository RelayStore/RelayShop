...
// Карта валют (заполняется из API)
let steamCurrencies = {};
...
function handleSteamLoginInput(e) {
    // Валидация: только латиница, цифры, _
...
async function handleSteamSubmit() {
    ...
    try {
        const result = await API.createSteamOrder(
            steamState.currency,
            login,
            steamState.amount
        );
        
        console.log('✅ Steam заказ выполнен:', result);
        
        if (result.status === 'completed') {
            alert(`✅ Пополнение Steam выполнено!\nСумма: ${steamState.amount} ${steamState.currency}\nСтатус: ${result.message || 'Успешно'}`);
        } else {
